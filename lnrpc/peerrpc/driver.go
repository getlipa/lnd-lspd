...
// createNewSubServer is a helper method that will create the new peer notifier
...
func createNewSubServer(configRegistry lnrpc.SubServerConfigDispatcher) (
	...
	// config.
	peerNotifierServerConf, ok := configRegistry.FetchConfig(subServerName)
	if !ok {
	...
	config, ok := peerNotifierServerConf.(*Config)
	if !ok {
		return nil, nil, fmt.Errorf("wrong type of config for "+
			"subserver %s, expected %T got %T", subServerName,
			&Config{}, peerNotifierServerConf)
	}

	// Before we try to make the new peer notifier service instance, we'll
	...
	case config.MacService != nil && config.NetworkDir == "":
		return nil, nil, fmt.Errorf("NetworkDir must be set to create " +
			"peerrpc")
	case config.PeerNotifier == nil:
		return nil, nil, fmt.Errorf("PeerNotifier must be set to " +
			"create peerrpc")
	}
